---
type: Test_program
function: relay_output_test (8-channel relay output test with pattern cycling and TFT display)
compatible_cpu_boards: [NORVI X-CPU-ESPS3-X1],[NORVI X-CPU-ESPS3-X2],[NORVI X-CPU-ESPS3-X3]
compatible_modules: [NORVI X-R8 (8-channel relay output expansion, PCA9538 I2C IO expander)]
status: completed
last_updated: 2026-08-17
---

# Example: NORVI X-R8 Relay Output Test

## What this does
Drives 8 relay outputs via a PCA9538 I2C IO expander through a repeating sequence of test patterns (All On/Off, Sequential, Odds vs Evens, Chaser, Binary Counter), showing the active pattern name and relay/state on the TFT screen while logging pattern names to Serial.

## Hardware required
- CPU board: NORVI X-CPU-ESPS3-X1 OR X2, X3 
- Expansion module(s): NORVI X-R8 (8-channel relay output module, PCA9538 I2C IO expander)
  - PCA9538 I2C address is DIP-switch selectable, default `0x72`, range `0x70`–`0x73` depending on DIP switch setting
...
| Relay 1–8 (via PCA9538) | PCA9538 pins 1, 2, 3, 4, 5, 6, 7, 8 |
...
- I2C scan results, e.g. `I2C device found at address 0x72  !`
...
On the TFT display: "NORVI" / "X-R8 TEST" header with a horizontal divider, followed by the active pattern name (e.g. "MODE: SEQUENTIAL"), then live relay states (RELAY 1–8 : ON/OFF) or "ALL ON"/"ALL OFF" or "VAL: n" depending on the current pattern, cycling continuously through all 5 patterns.

## Common Issues
| Symptom | Likely Cause | Fix |
|---|---|---|
| No I2C device found at 0x72 | PCA9538 not detected — wiring fault, DIP switch address doesn't match `PCA9538_ADDR`, or reset pin not held HIGH | Verify SDA/SCL on GPIO 8/9, confirm PCA_RESET (GPIO 21) is driven HIGH before Wire init, and check the module's DIP switch setting matches `0x72` (or update `PCA9538_ADDR` to match a different setting in the `0x70`–`0x73` range) |
| Relays don't switch even though TFT shows ON/OFF correctly | PCA9538 output not physically connected to relay driver, or relay board needs separate power supply | Check wiring from PCA9538 output pins to the X-R8 relay driver circuit and confirm relay board power |
| TFT shows nothing / blank screen | SPI pins (MISO 13 / MOSI 11 / SCLK 12) or Display CS (GPIO 45) miswired, or `tft.init()` failing silently | Verify SPI wiring and TFT_eSPI `User_Setup` pin configuration matches board pinout |
...
## Related Files
- Module reference: `expansion_module_R8.md`
